...
macro_rules! count_depth_limits {
    ( $( { $( [ $( $outer:ident : ( $( $inner:ident )* ) )* ] )* } )* ) => {
        [
            (${count(inner)}, ${count(inner, 1)}, ${count(inner, 2)}, ${count(inner, 3)}, ${count(inner, 4)}),
            (${count(outer)}, ${count(outer, 1)}, ${count(outer, 2)}, ${count(outer, 3)}, ${count(outer, 4)}),
        ]
    };
}
...
macro_rules! enumerate_literals_2 {
    ( $( [ $( ($l:literal) ),* ] ),* ) => {
        [$( $( (${index(1)}, ${length(1)}, ${index(0)}, ${length(0)}, $l), )* )*]
    };
}
...
/// to the count.
///
/// This macro uses dollar escaping to make it unambigious as to which
...
/// from a sequnce of inputs by their identifier.
///
/// This macro uses dollar escaping to make it unambigious as to which
